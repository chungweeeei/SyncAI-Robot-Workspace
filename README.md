...
Temporal-backed task orchestration backend whose REST / WebSocket API drives
the operator console (developed in its own repository since 2026-09; it is no
...
| `syncai_backend` | Python. FastAPI + rclpy in one process (port **3000**), Temporal worker (tasks, templates, schedules), map catalogue + pcd → gridmap conversion, TTS, WebSocket streams — the API the operator console is built on |

The Next.js operator console (`syncai_frontend`, formerly on port 3001) moved
to its own repository in 2026-09; nothing in this workspace serves it now.
...
The robot exposes the backend REST / WebSocket API at `http://<robot>:3000`
(interactive docs at `/docs`), the Temporal UI at `:8081` and pgAdmin at
`:5050`. Point the operator console (its own repository — nothing on the robot
serves it) at that backend; navigation goals, mode switches, teleop, and map
saving all go through it, and a raw `NavigateToPose` goal to
`/<robot_id>/task_runner` works too.
...
drive the robot, then save the map from the console
