...
#ifndef SYNCAI_PLANNER__PLUGINS__SMAC_PLANNER__UTILS_HPP_
#define SYNCAI_PLANNER__PLUGINS__SMAC_PLANNER__UTILS_HPP_
...
inline geometry_msgs::msg::Pose getWorldCoords(
  const float & mx, const float & my, const syncai_costmap_2d::Costmap2D * costmap)
{
...
*/
inline double findCircumscribedCost(std::shared_ptr<syncai_costmap_2d::Costmap2DROS> costmap)
{
  double result = -1.0;
  bool inflation_layer_found = false;
  std::vector<std::shared_ptr<syncai_costmap_2d::Layer>>::iterator layer;
  ...
  for (layer = costmap->getLayeredCostmap()->getPlugins()->begin();
    ...
    ++layer)
  {
    std::shared_ptr<syncai_costmap_2d::InflationLayer> inflation_layer =
      std::dynamic_pointer_cast<syncai_costmap_2d::InflationLayer>(*layer);
    if (!inflation_layer) {
      continue;
    }
...
// The upstream fromJson*() helpers lived here — removed with the
// SmacPlannerLattice plugin (they parsed its motion-primitive JSONs and were
// the package's only nlohmann_json use).

}  // namespace syncai_planner

#endif  // SYNCAI_PLANNER__PLUGINS__SMAC_PLANNER__UTILS_HPP_
