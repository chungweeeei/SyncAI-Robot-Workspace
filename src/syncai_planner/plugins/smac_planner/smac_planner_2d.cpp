...
#include "syncai_planner/plugins/smac_planner/smac_planner_2d.hpp"
...
SmacPlanner2D::~SmacPlanner2D()
{
  // Upstream reset this in deactivate(). Here the destructor is the only
  // teardown hook, and it must run before members are destroyed: the callback
  // captures `this`, so a parameter set racing destruction would call into a
  ...
  std::shared_ptr<syncai_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // Upstream registers this in activate(); with the single-phase initialize()
  // the plugin is live from here, so hook up dynamic parameters last — after
  // every member the callback re-initializes exists.
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  std::unique_lock<syncai_costmap_2d::Costmap2D::mutex_t> lock(*(_costmap->getMutex()));

  // Downsample costmap, if required
  syncai_costmap_2d::Costmap2D * costmap = _costmap;
  ...
  if (mx_start == mx_goal && my_start == my_goal) {
    if (costmap->getCost(mx_start, my_start) == syncai_costmap_2d::LETHAL_OBSTACLE) {
...
PLUGINLIB_EXPORT_CLASS(syncai_planner::SmacPlanner2D, syncai_nav_core::GlobalPlanner)
