  ...
  const double & possible_inscribed_cost)
{
  ...
  for (unsigned int i = 0; i != angles_.size(); i++) {
    ...
    syncai_costmap_2d::Footprint oriented_footprint;
  ...
  const bool & traverse_unknown)
{
  ...
  if (!footprint_is_radius_) {
    ...
    if (footprint_cost_ < possible_inscribed_cost_) {
      if (possible_inscribed_cost_ > 0) {
        ...
        RCLCPP_ERROR_THROTTLE(
          ...
          "github.com/ros-planning/navigation2/tree/main/syncai_planner#potential-fields"
    ...
    geometry_msgs::msg::Point new_pt;
    const syncai_costmap_2d::Footprint & oriented_footprint = oriented_footprints_[angle_bin];
    syncai_costmap_2d::Footprint current_footprint;
