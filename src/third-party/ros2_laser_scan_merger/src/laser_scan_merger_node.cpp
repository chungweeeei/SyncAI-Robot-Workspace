...
void scanMerger::init_parameters()
{
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] cloudTopic: '%s'", __func__, cloudTopic_.c_str());
  ...
  RCLCPP_INFO(this->get_logger(), "[scanMerger][%s] scanTopic1: '%s'", __func__, topic1_.c_str());
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] laser1XOff: '%f', laser1YOff: '%f', laser1ZOff: '%f'",
    __func__, laser1XOff_, laser1YOff_, laser1ZOff_);

  this->declare_parameter("laser1Alpha", 45.0);
  this->get_parameter_or<float>("laser1Alpha", laser1Alpha_, 0.0);
  RCLCPP_INFO(this->get_logger(), "[scanMerger][%s] laser1Alpha: '%f'", __func__, laser1Alpha_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] laser1AngleMin: '%f', laser1AngleMax: '%f'", __func__,
    laser1AngleMin_, laser1AngleMax_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] laser1R: '%d', laser1G: '%d', laser1B: '%d'", __func__,
    laser1R_, laser1G_, laser1B_);
  ...
  RCLCPP_INFO(this->get_logger(), "[scanMerger][%s] scanTopic2: '%s'", __func__, topic2_.c_str());
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] laser2XOff: '%f', laser2YOff: '%f', laser2ZOff: '%f'",
    __func__, laser2XOff_, laser2YOff_, laser2ZOff_);

  this->declare_parameter("laser2Alpha", 225.0);
  this->get_parameter_or<float>("laser2Alpha", laser2Alpha_, 0.0);
  RCLCPP_INFO(this->get_logger(), "[scanMerger][%s] laser2Alpha: '%f'", __func__, laser2Alpha_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] laser2AngleMin: '%f', laser2AngleMax: '%f'", __func__,
    laser2AngleMin_, laser2AngleMax_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "[scanMerger][%s] laser2R: '%d', laser2G: '%d', laser2B: '%d'", __func__,
    laser2R_, laser2G_, laser2B_);
...
void scanMerger::init_publisher()
{
  point_cloud_pub_ =
    this->create_publisher<sensor_msgs::msg::PointCloud2>(cloudTopic_, rclcpp::SensorDataQoS());
}

void scanMerger::init_subscriber()
{
  sub1_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
    topic1_, rclcpp::QoS(rclcpp::SensorDataQoS()),
    std::bind(&scanMerger::scan1_cb, this, std::placeholders::_1));

  sub2_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
    topic2_, rclcpp::QoS(rclcpp::SensorDataQoS()),
    std::bind(&scanMerger::scan2_cb, this, std::placeholders::_1));
}

void scanMerger::scan1_cb(const sensor_msgs::msg::LaserScan::SharedPtr _msg)
{
  laser1_ = _msg;
  update_point_cloud_rgb();
}

void scanMerger::scan2_cb(const sensor_msgs::msg::LaserScan::SharedPtr _msg)
{
  laser2_ = _msg;
}
