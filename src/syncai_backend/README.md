...
| POST | `/api/v1/tasks` | Start a `RobotWorkflow`; body is `{id, timestamp, steps[]}`. 409 while any task (direct or scheduled) is already running — one robot does one task at a time |
