...
| WS | `/api/v1/robot/teleop` | Inbound manual-control channel: client sends `{vx, vy, wz}` JSON frames at ~10 Hz, each normalized to [-1, 1]; the gateway clamps and scales to `TELEOP_MAX_*`. Refused (`{"error": ...}` frame, socket stays open) while an autonomous MOVE is executing. A 0.5 s stale-input watchdog and the disconnect path both publish zero velocity — the driver manager has no cmd_vel watchdog of its own |
