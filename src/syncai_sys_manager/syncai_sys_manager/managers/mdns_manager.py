...
from syncai_sys_manager.managers.wifi_manager import WifiNetworkInfo, WifiManager
...
class MdnsManager:
    ...
    def publish_mdns(self):
        ...
        mdns_domain = f"{self._conf_manager.get_robot_id()}.local"
        try:
        ...
        try:
            ...
            # rclpy's logger only accepts its own keyword args (throttle_*, once,
            # skip_first) and raises TypeError on anything else — so the return
            # code goes in the message, not a structlog-style kwarg.
            self._logger.error(
                f"[MdnsManager][publish_mdns] avahi-publish exited prematurely "
