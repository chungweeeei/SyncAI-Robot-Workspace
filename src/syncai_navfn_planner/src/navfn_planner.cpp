#include "syncai_navfn_planner/navfn_planner.hpp"
...
#include "syncai_navfn_planner/navfn.hpp"
#include "syncai_util/costmap.hpp"
...
namespace syncai_navfn_planner
{
  ...
  nav_msgs::msg::Path & plan)
{
  ...
  RCLCPP_INFO(
    logger_, "[NavfnPlanner][%s] Making plan from (%.2f,%.2f) to (%.2f,%.2f)", start.position.x,
    start.position.y, goal.position.x, goal.position.y, __func__);
  ...
  if (!worldToMap(wx, wy, mx, my)) {
    RCLCPP_WARN(
      logger_,
      "[NavfnPlanner][%s] Cannot create a plan: the robot's start position is off the global"
...
PLUGINLIB_EXPORT_CLASS(syncai_navfn_planner::NavfnPlanner, syncai_nav_core::GlobalPlanner)
