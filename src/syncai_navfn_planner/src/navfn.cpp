  ...
  float * plan, int nplan)
{
  ...
  if (len > 0) {  // found plan
    RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[NavFn][%s] Path found, %d steps\n", __func__, len);
  } else {
    RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[NavFn][%s] No path found\n", __func__);
  }
...
void NavFn::setGoal(int * g)
{
  ...
  goal[1] = g[1];
  RCLCPP_DEBUG(
    rclcpp::get_logger("rclcpp"), "[NavFn][%s] Setting goal to %d,%d\n", __func__, goal[0],
    goal[1]);
}
...
void NavFn::setStart(int * g)
{
  ...
  start[1] = g[1];
  RCLCPP_DEBUG(
    rclcpp::get_logger("rclcpp"), "[NavFn][%s] Setting start to %d,%d\n", __func__, start[0],
    start[1]);
}
...
{
  RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[NavFn][%s] Array is %d x %d\n", __func__, xs, ys);
