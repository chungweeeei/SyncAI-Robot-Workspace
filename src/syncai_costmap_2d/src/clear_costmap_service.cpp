...
ClearCostmapService::ClearCostmapService(
  const rclcpp::Node::SharedPtr & node, Costmap2DROS & costmap)
: costmap_(costmap)
{
...
{
  RCLCPP_INFO(
    logger_, "%s", ("Received request to clear except a region the " + costmap_.getName()).c_str());
...
{
  RCLCPP_INFO(
    logger_, "%s", ("Received request to clear entirely the " + costmap_.getName()).c_str());
