...
double FootprintCollisionChecker<CostmapT>::lineCost(int x0, int x1, int y0, int y1) const
{
  ...
  for (syncai_util::LineIterator line(x0, y0, x1, y1); line.isValid(); line.advance()) {
...
template class FootprintCollisionChecker<std::shared_ptr<syncai_costmap_2d::Costmap2D>>;
template class FootprintCollisionChecker<syncai_costmap_2d::Costmap2D *>;
