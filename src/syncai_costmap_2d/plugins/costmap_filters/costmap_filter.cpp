...
{
  if (!node_) {
    throw std::runtime_error{"Failed to get node in CostmapFilter"};
  }
  ...
  try {
    ...
    // The costmap node lives in its own sub-namespace; resolve a relative
    // filter info topic against the PARENT namespace so it still points at
    // /<parent>/<filter_info_topic> (same convention as StaticLayer's map_topic).
    filter_info_topic_ = joinWithParentNamespace(filter_info_topic_);
    ...
    enable_service_ = node_->create_service<std_srvs::srv::SetBool>(
      name_ + "/toggle_filter",
      std::bind(
        &CostmapFilter::enableCallback, this, std::placeholders::_1, std::placeholders::_2,
        std::placeholders::_3));
