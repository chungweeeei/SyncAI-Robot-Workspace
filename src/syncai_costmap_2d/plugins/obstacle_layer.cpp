#include "syncai_costmap_2d/obstacle_layer.hpp"
...
#include "syncai_costmap_2d/costmap_math.hpp"

PLUGINLIB_EXPORT_CLASS(syncai_costmap_2d::ObstacleLayer, syncai_costmap_2d::Layer)

using syncai_costmap_2d::NO_INFORMATION;
using syncai_costmap_2d::LETHAL_OBSTACLE;
using syncai_costmap_2d::FREE_SPACE;

using syncai_costmap_2d::ObservationBuffer;
using syncai_costmap_2d::Observation;
...
void ObstacleLayer::onInitialize()
{
  ...
  if (!node_) {
    throw std::runtime_error{"Failed to get node in ObstacleLayer"};
  }
  ...
  while (ss >> source) {
    ...
    node_->get_parameter(
      name_ + "." + source + "." + "expected_update_rate", expected_update_rate);
    ...
    node_->get_parameter(name_ + "." + source + "." + "obstacle_max_range", obstacle_max_range);
    node_->get_parameter(name_ + "." + source + "." + "obstacle_min_range", obstacle_min_range);

    // get the raytrace ranges for the sensor
    double raytrace_max_range, raytrace_min_range;
    node_->get_parameter(name_ + "." + source + "." + "raytrace_min_range", raytrace_min_range);
    node_->get_parameter(name_ + "." + source + "." + "raytrace_max_range", raytrace_max_range);
    ...
    if (data_type == "LaserScan") {
      auto sub = std::make_shared<
        message_filters::Subscriber<sensor_msgs::msg::LaserScan, rclcpp::Node>>(
        node_, topic, custom_qos_profile, sub_opt);
      sub->unsubscribe();

      auto filter = std::make_shared<tf2_ros::MessageFilter<sensor_msgs::msg::LaserScan>>(
        *sub, *tf_, global_frame_, 50, node_->get_node_logging_interface(),
        node_->get_node_clock_interface(), tf2::durationFromSec(transform_tolerance));
      ...
      auto sub = std::make_shared<
        message_filters::Subscriber<sensor_msgs::msg::PointCloud2, rclcpp::Node>>(
        node_, topic, custom_qos_profile, sub_opt);
      sub->unsubscribe();
...
void ObstacleLayer::laserScanCallback(
  ...
  const std::shared_ptr<syncai_costmap_2d::ObservationBuffer> & buffer)
{
    ...
    it != observations.end(); ++it)
  {
    ...
    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
      ...
      const unsigned int dist =
        static_cast<unsigned int>(std::hypot(static_cast<double>(dx), static_cast<double>(dy)));
  ...
  syncai_costmap_2d::Costmap2D & master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  if (footprint_clearing_enabled_) {
    setConvexPolygonCost(transformed_footprint_, syncai_costmap_2d::FREE_SPACE);
  }
...
void ObstacleLayer::addStaticObservation(
  syncai_costmap_2d::Observation & obs, bool marking, bool clearing)
{
  if (marking) {
