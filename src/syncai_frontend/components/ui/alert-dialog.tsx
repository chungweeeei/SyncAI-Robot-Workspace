...
/*
 * Confirm-or-abort dialog, wrapping @base-ui/react's AlertDialog the same way
 * components/ui/sheet.tsx wraps its Dialog — same primitive family, same
 * data-slot / class conventions, so the two read as one library.
 *
 * An *alert* dialog rather than a plain one on purpose: it has no dismiss-by-
 * backdrop-click and no close button, so the only ways out are the two buttons.
 * Every use here is a question about moving a real machine, and a modal that a
 * stray click on the map can dismiss teaches the operator to click through it.
 */
