...
void Preprocess::process(
  const livox_ros_driver2::msg::CustomMsg::UniquePtr & msg, PointCloudXYZI::Ptr & pcl_out)
{
  avia_handler(msg);
  *pcl_out = pl_surf;
}
...
void Preprocess::process(
  const sensor_msgs::msg::PointCloud2::UniquePtr & msg, PointCloudXYZI::Ptr & pcl_out)
{
  switch (time_unit) {
    case SEC:
...
{
  /**
   * default handler 主要做的事情就是 
   * clear -> fromROSMsg → 逐點複製 → blind 過濾 → push_back 進 pl_surf
   **/
  ...
  if (has_intensity) {
    ...
    // xyz-only cloud (e.g. Isaac Sim): convert without the intensity field so
    // pcl doesn't warn "Failed to find match for field 'intensity'" every frame
    pcl::PointCloud<pcl::PointXYZ> pl_xyz;
    pcl::fromROSMsg(*msg, pl_xyz);
    pcl::copyPointCloud(pl_xyz, pl_orig);
  }
  int plsize = pl_orig.points.size();
  if (plsize == 0) return;
  pl_surf.reserve(plsize);
