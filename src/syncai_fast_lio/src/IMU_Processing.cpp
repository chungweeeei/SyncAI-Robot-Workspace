...
ImuProcess::ImuProcess() : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  ...
  // iEKF 所用到的 noise covariance matrix
  Q = process_noise_cov();
  ...
  PointCloudXYZI & pcl_out)
{
  ...
  /** 
   * Initialize IMU pose 
   * IMU的姿態存的資料是 EKF透過積分算出來的姿態軌跡
   * IMUPose中每個欄位分別為
   * - offset_time (tail_stamp - pcl_beg_time) 時間搓，直接計算
   * - gyr (angvel_last)，量測扣掉 bias
   * - acc (acc_s_last)，量測轉世界系、扣 bias、加重力 
   * - vel predict 算的
   * - pos predict 算的
   * - rot predict 算的
   * 後面3筆資料 vel、pos、rot這三個是透過 kf_state.predict()執行完後，imu_state = kf_state.get_x() 從 EKF 狀態裡取出來的
   **/
  state_ikfom imu_state = kf_state.get_x();
  IMUpose.clear();
  IMUpose.push_back(set_pose6d(
    0.0, acc_s_last, angvel_last, imu_state.vel, imu_state.pos, imu_state.rot.toRotationMatrix()));
  ...
  input_ikfom in;
  /**
   * forward propagation 主要是透過相鄰兩筆 IMU 的資料進行配對
   **/
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    if (head_stamp < last_lidar_end_time_) {
      dt = tail_stamp -
           last_lidar_end_time_;  // 跨幀第一段，head 是上一幀尾巴的那筆，但 tail 已經跨進到這一幀。
      // dt = tail->header.stamp.toSec() - pcl_beg_time;
    } else {
      dt = tail_stamp - head_stamp;  // 一般情況，dt就是兩筆 IMU 的時間差
    }
     ...
     * predict() 用的是一個以 IMU 量測為輸入的運動模型 (kinematic / process model)。IMU 的加速度和角速度在扮演的是「控制輸入u」 
     * kinematic model 的定義在 use-ikfom.hpp 這個 library中
     **/
  ...
  /**
   * 前面的 for 迴圈是「相鄰兩筆 IMU 配對」逐區間傳播，跑到 v_imu.end()-1 就停
   * —也就是傳播只推進到最後一筆 IMU 的時刻 imu_end_time。
   * 但掃描結束時刻 pcl_end_time 不一定剛好落在最後一筆 IMU 上，通常兩者有個小落差。這一段就是用最後的 IMU 輸入再補一小步，讓 EKF 狀態精確落在 pcl_end_time。
   **/
  double note = pcl_end_time > imu_end_time ? 1.0 : -1.0;
  dt = note * (pcl_end_time - imu_end_time);

  // 要將predict補償到對齊pcl_end_time
  kf_state.predict(dt, Q, in);
   ...
   * 1. 「機器人在 0.1 秒內移動了多少、轉了多少」→ 靠 IMU + 前向傳播算出來，存進 IMUpose。
   * 2. 「每個點是這一圈的第幾個時刻打出的」→ 存在每個點的 curvature 欄位（相對時間）。
   * 3. 「把點 A 從它量測的位置搬到結束位置」→ 就是那條 P_compensate 公式在做的座標換算。
   **/
  ...
  PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  /**
   * 依照 flag 判斷IMU資料是否需要靜置初始化：估重力、gyro bias等
   */
  if (imu_need_init_) {
