#ifndef SYNCAI_BEHAVIOR_TREE__BT_ACTION_SERVER_HPP_
#define SYNCAI_BEHAVIOR_TREE__BT_ACTION_SERVER_HPP_
...
{
public:
  using ActionServer = syncai_util::SimpleActionServer<ActionT>;
  ...
  typedef std::function<void(typename ActionT::Result::SharedPtr, syncai_behavior_tree::BtStatus)>
...
private:
  ...
  std::unique_ptr<syncai_behavior_tree::BehaviorTreeEngine> bt_;
  ...
  // Parent node (weak ref to avoid a circular reference with the owning node)
...
#include "syncai_behavior_tree/bt_action_server_impl.hpp"  // NOLINT(build/include_order)
#endif  // SYNCAI_BEHAVIOR_TREE__BT_ACTION_SERVER_HPP_