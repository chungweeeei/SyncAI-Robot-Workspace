#ifndef SYNCAI_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
#define SYNCAI_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
...
template <class ActionT>
void BtActionServer<ActionT>::initialize()
{
  auto node = node_.lock();
  if (!node) {
    throw std::runtime_error{"Failed to lock node"};
...
void BtActionServer<ActionT>::executeCallback()
{
  ...
  syncai_behavior_tree::BtStatus rc = bt_->run(&tree_, on_loop, is_canceling, bt_loop_duration_);
...
#endif  // SYNCAI_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
