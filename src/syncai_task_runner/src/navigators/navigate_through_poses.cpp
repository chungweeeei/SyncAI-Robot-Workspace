...
std::string NavigateThroughPosesNavigator::getDefaultBTFilepath(rclcpp::Node::WeakPtr parent_node)
{
  ...
  if (!node->has_parameter("default_nav_through_poses_bt_xml")) {
    ...
    node->declare_parameter<std::string>(
      "default_nav_through_poses_bt_xml", pkg_share_dir + "/behavior_trees/patrol.xml");
  }
...
bool NavigateThroughPosesNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  if (!bt_action_server_->loadBehaviorTree(bt_xml_filename)) {
    RCLCPP_ERROR(
      logger_,
      "[NavigateThroughPosesNavigator][%s] Error loading XML file: %s. Navigation canceled.",
...
{
  RCLCPP_INFO(
    logger_, "[NavigateThroughPosesNavigator][%s] Received goal preemption request", __func__);
  ...
  if (
    ...
    RCLCPP_WARN(
      logger_,
      "[NavigateThroughPosesNavigator][%s] Preemption request was rejected since the requested BT "
      "XML file is not the same "
...
void NavigateThroughPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  if (goal->poses.size() > 0) {
    RCLCPP_INFO(
      logger_,
      "[NavigateThroughPosesNavigator][%s] Begin navigating from current location through %zu "
      "poses to (%.2f, %.2f)",
      __func__, goal->poses.size(), goal->poses.back().pose.position.x,
