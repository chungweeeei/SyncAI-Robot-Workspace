...
#include "syncai_util/occ_grid_values.hpp"
...
void loadMapFromFile(const LoadParameters & load_parameters, nav_msgs::msg::OccupancyGrid & map)
{
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("map_io"), "Loading image_file: " << load_parameters.image_file_name);
  ...
  Eigen::Map<Eigen::Matrix<uint8_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> gray_matrix(
    buffer.data(), height, width);
  ...
  if (load_parameters.mode == MapMode::Trinary || load_parameters.mode == MapMode::Scale) {
    ...
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> normalized =
    ...
    result.setConstant(syncai_util::OCC_GRID_UNKNOWN);

    // Apply occupied and free cell updates
    result = (occupied.array() > 0).select(syncai_util::OCC_GRID_OCCUPIED, result);
    result = (free.array() > 0).select(syncai_util::OCC_GRID_FREE, result);
    ...
    if (load_parameters.mode == MapMode::Scale) {
      ...
      if (in_between_mask.any()) {
        ...
        Eigen::ArrayXXf scaled_float =
          ((normalized.array() - load_parameters.free_thresh) /
    ...
    if (has_alpha) {
      ...
      Eigen::Map<Eigen::Array<uint8_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>
        alpha_array(alpha_buf.data(), height, width);

      // Apply mask directly with Eigen::select
      result = (alpha_array < 255).select(syncai_util::OCC_GRID_UNKNOWN, result);
    }
    ...
    auto out_of_bounds = (result.array() < syncai_util::OCC_GRID_FREE) ||
                         (result.array() > syncai_util::OCC_GRID_OCCUPIED);

    result = out_of_bounds.select(syncai_util::OCC_GRID_UNKNOWN, result);
  ...
  Eigen::Matrix<int8_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> flipped =
    result.colwise().reverse();
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("map_io"), "Read map " << load_parameters.image_file_name << ": "
                                              << msg.info.width << " X " << msg.info.height
                                              << " map @ " << msg.info.resolution << " m/cell");
...
LoadParameters loadMapYaml(const std::string & yaml_filename)
{
  ...
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("map_io"), "occupied_thresh: " << load_parameters.occupied_thresh);
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("map_io"), "mode: " << map_mode_to_string(load_parameters.mode));
...
LOAD_MAP_STATUS loadMapFromYaml(const std::string & yaml_file, nav_msgs::msg::OccupancyGrid & map)
{
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("map_io"), "Failed processing YAML file "
                                      << yaml_file << " at position (" << e.mark.line << ":"
                                      << e.mark.column << ") for reason: " << e.what());
    return INVALID_MAP_METADATA;
  } catch (std::exception & e) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("map_io"),
      "Failed to parse map YAML loaded from file " << yaml_file << " for reason: " << e.what());
    return INVALID_MAP_METADATA;
  }
  try {
    ...
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("map_io"), "Failed to load image file " << load_parameters.image_file_name
                                                                 << " for reason: " << e.what());
...
void checkSaveParameters(SaveParameters & save_parameters)
{
  ...
  if (save_parameters.map_file_name == "") {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("map_io"),
      "Map file unspecified. Map will be saved to " << save_parameters.map_file_name << " file");
  }
  ...
  if (save_parameters.occupied_thresh == 0.0) {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("map_io"), "Occupied threshold unspecified. Setting it to default value: "
  ...
  if (save_parameters.free_thresh == 0.0) {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("map_io"),
      "Free threshold unspecified. Setting it to default value: " << save_parameters.free_thresh);
  }
  ...
  if (save_parameters.occupied_thresh <= save_parameters.free_thresh) {
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("map_io"), "Threshold_free must be smaller than threshold_occupied");
    throw std::runtime_error("Incorrect thresholds");
  }
  ...
  if (save_parameters.image_format == "") {
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("map_io"),
      "Image format unspecified. Setting it to: " << save_parameters.image_format);
  }
  ...
  if (
    ...
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("map_io"), "Requested image format '"
...
{
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("map_io"), "Received a " << map.info.width << " X " << map.info.height
  ...
  std::string mapdatafile = save_parameters.map_file_name + "." + save_parameters.image_format;
  {
    ...
    for (size_t y = 0; y < map.info.height; y++) {
      for (size_t x = 0; x < map.info.width; x++) {
        ...
        switch (save_parameters.mode) {
          ...
          default:
            RCLCPP_ERROR_STREAM(
              rclcpp::get_logger("map_io"), "Map mode should be Trinary, Scale or Raw");
            throw std::runtime_error("Invalid map mode");
        }
    ...
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("map_io"), "Writing map occupancy data to " << mapdatafile);
  ...
  std::string mapmetadatafile = save_parameters.map_file_name + ".yaml";
  {
    ...
    e << YAML::Key << "resolution" << YAML::Value
      << to_string_with_precision(map.info.resolution, 3);
    ...
    if (!e.good()) {
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("map_io"), "YAML writer failed with an error "
                                        << e.GetLastError()
...
bool saveMapToFile(const nav_msgs::msg::OccupancyGrid & map, const SaveParameters & save_parameters)
{
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(
      rclcpp::get_logger("map_io"), "Failed to write map for reason: " << e.what());
    return false;
  }
  return true;
}
